...
import { testSet } from "../__setups__/setdata";
import { CompletedType, testExercise } from "../__setups__/exercisedata";
import {
  ExerciseItemType,
  SetItemType,
...
describe("Exercise Provider functions", () => {
  ...
  testExercise(
    "check one exercise is completed or not",
    ({ exercises }: any) => {
  ...
  testExercise(
    "find next excercise not completed",
    ({ nextExercises }: any) => {
  ...
  testExercise(
    ...
    ({ nextExercises }: any) => {
      ...
      testErcises.forEach((testEx, i) => {
        ...
        let setStarted = exercise.sets.filter(
          (set: SetItemType) => set?.started
        );
